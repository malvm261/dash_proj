...
{
    // ════════════════════════════════════════════════════════════════
    //  Главная форма диспетчера распределённого расчёта.
    //
    //  Общий объём вычислений (число членов ряда) делится на непересекающиеся
    //  диапазоны — по одному на каждый воркер из списка адресов. Каждый воркер
    //  получает СВОЙ диапазон через POST /api/start и считает только его,
    //  используя все свои ядра. Диспетчер периодически опрашивает воркеры через
    //  GET /api/status и СКЛАДЫВАЕТ их частичные суммы в одно общее значение π.
    //
    //  Поэтому чем больше воркеров участвует в расчёте — тем меньше работы
    ...
    public partial class MainForm : Form
    {
        ...
        // Воркер отдаёт JSON в PascalCase — указываем то же на диспетчере на всякий случай
        ...
        // Последний полученный статус каждого воркера — нужен для пересчёта общего π
        ...
        private async void BtnStart_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Укажите хотя бы один адрес воркера (например, http://192.168.1.10:5050).",
            ...
            // Делим общий диапазон [0 .. totalIterations) поровну между воркерами —
            ...
            // Каждому воркеру отправляем ЕГО СОБСТВЕННЫЙ диапазон — это и есть декомпозиция задачи
            foreach (var a in assignments)
            {
                ...
                {
                    MessageBox.Show($"Не удалось отправить задание на воркер:\n{a.Address}\n\n{ex.Message}",
        ...
        {
            SetStatus("Остановка воркеров...", Color.Gray);
            _cts?.Cancel();

            foreach (var address in _rows.Keys)
            {
                try { await _http.PostAsync($"{address}/api/stop", content: null); }
                catch { /* воркер недоступен — пропускаем, это не критично */ }
            }
        }
        ...
        private async Task PollLoop(List<string> addresses, long totalIterations, int intervalSeconds, CancellationToken ct)
        {
            while (!ct.IsCancellationRequested)
            {
                ...
                // Складываем частичные суммы со всех воркеров в одно общее значение π
                RecalculateCombinedResult(totalIterations);

                if (everyNodeFinished)
                {
                    SetStatus("Распределённый расчёт завершён на всех воркерах", Color.SeaGreen);
                    return;
                }
        ...
        private void ShowNodeUnavailable(string address)
        {
            ...
            row.Cells[2].Value = "—";
            row.Cells[3].Value = "воркер недоступен";
            row.DefaultCellStyle.ForeColor = Color.Firebrick;
        }

        // Складывает частичные суммы рядов со всех воркеров в одно общее значение π
    ...
    // ════════════════════ Модели обмена данными (должны совпадать с воркером) ════════════════════
    ...
    // (это не значения π, а вклад только этого воркера — итоговое π диспетчер получает,
    // складывая частичные суммы со всех воркеров)
